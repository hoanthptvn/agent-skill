...
>
> 1. **Cấm định vị sai mục đích:** Tuyệt đối cấm lạm dụng `position: absolute` và tính toán `margin` thủ công để dàn trang. BẮT BUỘC dùng `display: grid` hoặc `flex` cho layout 1D/2D.
> 2. **Cấm Fixed Width:** Không được set `width: 1200px` cố định. Phải dùng các thuộc tính Logical (như `max-inline-size`) kết hợp với `width: 100%` để tự động Responsive.
> 3. **Cấm đẻ quá nhiều Breakpoints:** Hạn chế viết `@media (max-width: 768px)` lắt nhắt ở mọi nơi. Hãy ưu tiên thiết kế nội tại (Intrinsic Design) dùng `flex-wrap`, `grid-template-columns: repeat(auto-fit, ...)`.
