...
description: Kỹ năng Agent chuyên biệt về cay-state-machine cho hệ thống CSS.
---
...
- **Hoạt động:** Tác tử Critic đọc PR và so chiếu với `skills/loops/` và `skills/types/`. Nếu phát hiện `forEach` trong animation loop, nó tự động **Block PR**.
