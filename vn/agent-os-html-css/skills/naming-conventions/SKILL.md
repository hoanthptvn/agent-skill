...
/* ── ACTION COLORS ───────────────────────── */
--ui-primary:          /* Brand color */ --ui-secondary: /* Secondary action */
  --ui-success: /* Success state */ --ui-warning: /* Warning state */
  --ui-error: /* Error state */ --ui-info: /* Info state */;
```
...
`css
--ui-{property} /* Biến composite cho property CSS _/
--ui-{property}-{modifier} /_ Sub-property */
...
.shadow-md {
  --ui-shadow:
    0 4px 6px -1px var(--ui-shadow-color, #0000001a),
    0 2px 4px -2px var(--ui-shadow-color, #0000001a);
  box-shadow:
    var(--ui-inset-shadow),
    /* always valid */ var(--ui-inset-ring-shadow),
    /* always valid */ var(--ui-ring-offset-shadow),
    /* always valid */ var(--ui-ring-shadow),
    /* always valid */ var(--ui-shadow); /* ← vừa set */
}
...
.ring-2 {
  ...
  box-shadow:
    ...
    var(--ui-ring-shadow),
    /* ← vừa set */ var(--ui-shadow); /* ← giữ từ .shadow-md */
}
```
...
.blur {
  --ui-blur: blur(8px);
  filter: var(--ui-blur,) var(--ui-brightness,) var(--ui-contrast,)
    var(--ui-grayscale,) var(--ui-hue-rotate,) var(--ui-invert,)
    var(--ui-saturate,) var(--ui-sepia,) var(--ui-drop-shadow,);
}

/* Ghép thêm grayscale — không mất blur! */
.grayscale {
  --ui-grayscale: grayscale(100%);
  filter: var(--ui-blur,) /* ← giữ blur(8px) */ var(--ui-brightness,)
    var(--ui-contrast,) var(--ui-grayscale,) /* ← vừa set */...;
}
```
...
## Quy tắc Cú pháp BEM & Class Order

### Cú pháp BEM Chuẩn

- **Element** (`__`): 2 gạch dưới. **TUYỆT ĐỐI KHÔNG lồng Element**.
  - ❌ Sai: `.c-card__body__text`
  - ✅ Đúng: `.c-card__text` (Tất cả element đều là con trực tiếp của block về mặt naming).

### Class Order trên HTML

Thứ tự các class trên 1 thẻ HTML phải luôn theo logic từ "To" đến "Nhỏ" để dễ đọc:
`[Layout] → [Component] → [Modifier] → [Visual State] → [Utility]`
Ví dụ: `class="l-stack c-card c-card--glass is-active u-m-4"`

---

## Page Namespace Pattern (`.p-`)
...
## Component Naming Dictionary (Từ điển Đặt tên 6 Nhóm)
...
| Custom Checkbox        | `.c-checkbox-custom`          | Elements: `__input` `__icon` `__label`                                               |
| Radio Group / Radio    | `.c-radio-group` / `.c-radio` | Elements: `__input` `__label`                                                        |
| Switch / Toggle        | `.c-switch`                   |                                                                                      |
| Toggle Button          | `.c-btn-toggle`               |                                                                                      |
| Slider                 | `.c-slider`                   |                                                                                      |
| Rating                 | `.c-rating`                   |                                                                                      |
| Number Field           | `.c-input-number`             |                                                                                      |
...
| Tooltip    | `.c-tooltip` | Elements: `__body` `__arrow`                                |
...
| Progress (tròn)  | `.c-progress-circular` |                                                      |
| Progress (ngang) | `.c-progress-bar`      |                                                      |
| Skeleton         | `.c-skeleton`          |                                                      |
| Backdrop         | `.c-backdrop`          | z-index: 1000                                        |
...
| Drawer / Sidebar | `.c-drawer`    | Modifier: `--left` `--right`                                                                                        |
...
| Bottom Navigation  | `.c-nav-bottom`  | Elements: `__item` `__icon` `__label`                     |
| Menu / Dropdown    | `.c-menu`        | Elements: `__list` `__item` `__link`                      |
| Breadcrumbs        | `.c-breadcrumbs` | Elements: `__list` `__item` `__link`                      |
| Link               | `.c-link`        |                                                           |
| Stepper            | `.c-stepper`     | Elements: `__item` `__step` `__label`                     |
| Pagination         | `.c-pagination`  | Elements: `__list` `__item` `__link`                      |

### Nhóm 6 — Quy tắc 3 Tầng Naming & Semantic Suffix Block

#### Decision Tree — Khi nào Modifier, khi nào Block mới?

Khi cùng "loại" component (VD: Card) xuất hiện nhiều nơi với design khác nhau, áp dụng theo thứ tự:

```
Cùng cấu trúc HTML, chỉ thay màu/size/spacing?
├── CÓ → Modifier: .c-card.c-card--featured
│
├── KHÔNG — cấu trúc khác, gắn với domain cụ thể?
│   └── CÓ → Semantic Suffix Block: .c-card-team / .c-card-price
│
└── Hoàn toàn độc lập, không liên quan tới card?
    └── Block mới: .c-article, .p-hero-feature
```

#### Tầng 1 — Modifier (Biến thể thị giác)

Dùng khi **cấu trúc HTML giống nhau**, chỉ thay màu sắc, kích thước, trạng thái. Luôn kèm class gốc. Modifier CHỈ override biến — không viết lại toàn bộ style.

```html
<!-- ✅ ĐÚNG: Modifier đi cùng base -->
<article class="c-card c-card--popular">...</article>
<article class="c-card c-card--featured">...</article>

<!-- ❌ SAI: Modifier đứng một mình (mất style gốc) -->
<article class="c-card--popular">...</article>
```

#### Tầng 2 — Semantic Suffix Block (Block theo Ngữ nghĩa)

Dùng khi cùng "loại" component nhưng **cấu trúc HTML khác biệt**, gắn với một domain cụ thể. Đặt tên theo **chức năng/ngữ nghĩa nội dung**, không theo vị trí trang.
**Cú pháp:** `.c-card-[domain]` (dùng chung nhiều trang) · `.p-card-[domain]` (chỉ dùng 1 trang).
...
| `.c-card-service` | Giới thiệu dịch vụ     | `__icon` `__title` `__desc`         |
| `.c-card-case`    | Case study / dự án     | `__cover` `__client` `__tag`        |
| `.c-card-stat`    | Số liệu / thống kê     | `__value` `__label` `__trend`       |

#### Tầng 3 — Modifier trên Semantic Block

Khi Semantic Block cũng cần biến thể riêng:

```html
<!-- Card giá nổi bật nhất -->
<article class="c-card-price c-card-price--popular">...</article>

<!-- Card sản phẩm đã bán hết -->
<article class="c-card-product c-card-product--sold-out">...</article>
```
