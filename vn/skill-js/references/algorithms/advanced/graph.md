...
Graph mô phỏng mạng lưới thực tế: Google Maps, Facebook, Webpack Dependency Graph, React Component relationships.
...
Frontend use cases:
  ...
  Node-based UI: React Flow, Blender Material = Graph visualization
```
...
Dependency Graph (Recoil/Jotai): khi A đổi → tính B, C phụ thuộc → re-render chính xác
N8N / Figma Prototyping / React Flow = Graph visualization (kéo thả nodes + edges)
...
Immutability với React State:
  Map.set() / Set.add() mutate trực tiếp → React bỏ qua re-render
  Dùng Immer.js (enableMapSet()) → viết code mutate bình thường
  Immer tự trả về Immutable State mới → React nhận ra thay đổi → re-render
```
