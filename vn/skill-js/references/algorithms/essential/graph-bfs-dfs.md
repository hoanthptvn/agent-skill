...
DOM = Cây. React Fiber = Cây. Three.js Scene = Graph. Mọi hiệu ứng phức tạp đều quy về duyệt cây/đồ thị.
...
```
PreOrder:  Cha → Trái → Phải  → Clone, Serialize, React diff
...
PreOrder (Cha TRƯỚC):
  ...
  React Virtual DOM Diff: Cha diff trước, memo match → skip cả nhánh
