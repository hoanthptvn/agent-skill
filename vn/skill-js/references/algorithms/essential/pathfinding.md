...
```
React useMemo / useCallback = DP Memoization:
  props không đổi (Overlapping Subproblems) → dùng cached UI
  props đổi → tính lại rồi cache

Virtual DOM Diffing (React Reconciliation):
  Bài toán gốc: so sánh 2 cây DOM = DP (Longest Common Subsequence) O(N³)
  React Heuristics (Greedy): giả định key và tên thẻ không đổi → O(N)
...
DP trong State Management:
  Recoil/Jotai Dependency Graph: khi A thay đổi → tính toán B, C phụ thuộc
...
// Frontend analogies:
//   Memoization = React useMemo/useCallback/React.memo
