...
// Khi React/Vue unmount component → element bị GC
...
// Immer.js + React — enableMapSet()
// Map.set() / Set.add() mutate trực tiếp → React bỏ qua re-render
// Immer tự trả về Immutable State mới → React nhận ra thay đổi
...
KHÔNG làm:
  ...
  Map/Set.add() trực tiếp trong React state → Immer enableMapSet()
```
